...
## Audit log
Create a run directory `.cursor/lazysitter/runs/<slug>/` (slug from the feature). Write:
...
- **0a.** Check the kill switch. Read the process pitfall ledger `.cursor/lazysitter/PITFALL-LEDGER.md` so you don't repeat a known process fault.
...
Summarize: what was built, the triage size + which agents woke, the merge-gate verdict (from `gate-state.jsonl`), any logged overrides, red-team/security findings, **known limitations** (`LIMITATIONS.md`), any `degraded`/accepted-risk items, graduation candidates, rollout mode, and the audit-log path. Keep it skimmable; point to `.cursor/lazysitter/runs/<slug>/` for detail.
