...
- **Kill switch:** before each tier, check for the file `.claude/newronaut/KILL`. If it exists, halt immediately, write a final audit entry, and stop. Tell the user the kill switch was tripped.
...
## Escalation
The ONLY downstream reason to ask the user is the budget cap. The only *intake* reason is business/scope ambiguity surfaced by `newronaut-business-analyst` (relay its `CLARIFY` block via `AskUserQuestion`, feed answers back). Every other disagreement resolves via architect ruling + logged override — never escalated.

## Audit log
Create a run directory `.claude/newronaut/runs/<slug>/` (slug from the feature). Write:
...
### Tier 1 — Intake
1. Spawn `newronaut-business-analyst` with the raw request. Save `REQUIREMENT.md`. If it returns a `CLARIFY` block, ask the user those questions via `AskUserQuestion`, then re-run or amend the requirement with the answers.
...
### Tier 2 — Research
3. Spawn `newronaut-explorer`. Save `CONTEXT-PACK.md`. This single pack is reused by everyone downstream — do not let any later agent re-explore.
...
5. Spawn `newronaut-architect` to draft the plan, plus the triage-selected experts from {`newronaut-database-expert`, `newronaut-infra-expert`, `newronaut-frontend-expert`, `newronaut-ux-analyst`} and ALWAYS `newronaut-security-expert`. Experts report to the architect only — collect their opinions and hand them to the architect; do not let experts talk to each other.
6. **Every round**, also spawn `newronaut-devils-advocate` against the current leading position and give its challenge to the architect — even if the panel already agrees (fast unopposed convergence is not trusted).
...
12. Spawn together: `newronaut-test-runner` (frozen tests vs implementation), `newronaut-code-reviewer` (diff vs plan + lint/typecheck/build), `newronaut-red-team` (attack it), `newronaut-security-auditor` (audit the diff; give it the design-expert's post-build hand-off list), and `newronaut-secrets-scanner` (diff). Save each report.
...
18. Spawn `newronaut-release-agent`: rebase onto current devBase, re-verify the gate, and prefer staged/canary rollout if infra supports it (per infra-expert). Record the merge ref.
19. Spawn `newronaut-monitor-agent` for the monitoring window on the merge ref.
20. If the monitor reports `REGRESSION`, spawn `newronaut-rollback-agent` immediately (standing authority — no extra approval).
21. If stable, spawn `newronaut-docs-agent` to update changelog/README/API docs.

## Final report to the user
Summarize: what was built, the triage size + which agents woke, the merge-gate verdict, any logged overrides, red-team/security findings, rollout mode, and the audit-log path. Keep it skimmable; point to `.claude/newronaut/runs/<slug>/` for detail.
