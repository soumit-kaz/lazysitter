...
function manifestFor(dir) {
  const p = path.join(path.resolve(dir || process.cwd()), '.provenforge', 'manifest.json');
...
function listRoster() {
  ...
  log.info('');
  log.info(`${c.bold('Provenforge roster')} — ${agents.length} agents`);
  log.info('');
...
function help() {
  log.info(`
${c.bold('provenforge')} — install the Autonomous Engineering Team into a project (Claude Code + Codex)

${c.bold('Usage')}
  npx provenforge <command> [dir] [flags]

${c.bold('Commands')}
  init [dir]        Install Provenforge into a project (default). Auto-detects Claude/Codex.
  update [dir]      Refresh managed files; keeps your models.env / config edits.
  uninstall [dir]   Remove Provenforge. Add --purge to also delete your config.
...
${c.bold('Flags')}
  ...
  --purge           (uninstall) also remove user config (models.env, provenforge.config.json).
  --force           Overwrite without prompting.

${c.bold('Examples')}
  npx provenforge init                 ${c.dim('# both adapters, into the current repo')}
  npx provenforge init . --codex       ${c.dim('# Codex only')}
  npx provenforge doctor
  npx provenforge uninstall --purge
`);
}
