...
const { NEWTON_BEGIN, NEWTON_END } = require('./context');
...
function uninstall(pkgRoot, opts) {
  ...
  if (!exists(manifestPath)) {
    log.err(`No Newton install found in ${targetRoot} (missing .newton/manifest.json).`);
    process.exitCode = 1;
    return;
  }
  ...
  log.info('');
  log.info(`${c.bold('Removing Newton')} from ${c.bold(targetRoot)}`);
  log.info('');
  ...
  // Remove the manifest and any now-empty Newton directories.
  fs.rmSync(manifestPath);
  dirs.add(path.dirname(manifestPath));
  removeEmptyDirsUpward(dirs, targetRoot);

  log.info('');
  log.ok(`${c.bold('Newton removed.')}`);
  log.info('');
}
...
function stripAgentsMd(targetRoot, info) {
  ...
  const b = text.indexOf(NEWTON_BEGIN);
  const e = text.indexOf(NEWTON_END);
  if (b !== -1 && e !== -1) {
    text = (text.slice(0, b) + text.slice(e + NEWTON_END.length)).replace(/\n{3,}/g, '\n\n').trimStart();
  }
  if (info.createdByAet && text.trim() === '') {
    ...
    log.ok(`  stripped Newton block from ${info.path}`);
  }
}
