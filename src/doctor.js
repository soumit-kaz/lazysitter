...
function doctor(pkgRoot, opts) {
  ...
  log.info('');
  log.info(`${c.bold('Provenforge doctor')} — ${targetRoot}`);
  log.info('');

  if (!exists(manifestPath)) {
    log.err('No Provenforge install found here (.provenforge/manifest.json missing).');
    log.info(`  Run ${c.dim('npx provenforge init')} first.`);
    process.exitCode = 1;
    return;
  }
  ...
  else log.info(`  ${c.dim(`${missing} missing, ${drifted} locally modified (re-run \`provenforge update\` to restore)`)}`);
  log.info('');
  ...
  if (manifest.tools.includes('codex')) {
    const modelsEnv = path.join(targetRoot, '.codex/skills/provenforge/models.env');
    if (exists(modelsEnv)) {
      ...
      if (!alt || !alt[1].trim()) {
        ...
        log.info(`    ${c.dim('Set a distinct high-tier slug in .codex/skills/provenforge/models.env')}`);
      } else {
...
function checkBinary(bin, label) {
  try {
    ...
    log.warn(`  ${label} not found on PATH (${bin}) — install it to run Provenforge.`);
  }
}
