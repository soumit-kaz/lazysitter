...
// The index is derived, machine-local and regenerable, so it must never be
// committed. The ignore file is written by whatever creates the directory —
// not only by the installer — so an index built directly is still covered.
const INDEX_GITIGNORE = `# Generated by \`lazysitter fe-index build\`. Derived and regenerable — never commit it.
# This ignores ONLY .lazysitter/index/. The sibling .lazysitter/knowledge/ is
# committed institutional memory and stays tracked.
*
!.gitignore
`;
...
function ensure(root) {
  ...
  if (!fs.existsSync(ignore)) {
    try {
      fs.writeFileSync(ignore, INDEX_GITIGNORE, 'utf8');
    } catch {}
  }
  return dir;
}
...
function clear(root) {
  ...
  for (const f of fs.readdirSync(dir)) {
    // Keep the ignore file: a rebuild after `clear` would otherwise drop a
    // directory of generated JSON straight into `git status`.
