...
const SKILL_DIR = '.codex/skills/newronaut';
...
function installCodex(ctx, data) {
  log.step('Codex adapter → .codex/skills/newronaut/');
  ...
  ctx.writePreserve(
    `${SKILL_DIR}/newronaut.config.json`,
    readFile(path.join(ctx.coreDir, 'codex', 'newronaut.config.json'))
  );

  // Discovery guarantee: point AGENTS.md at the skill.
  ctx.mergeMarkedBlock('AGENTS.md', readFile(path.join(ctx.coreDir, 'codex', 'AGENTS.newronaut.md')));
}

function roleFile(agent) {
  const header = `<!-- Newronaut role: ${agent.name} · tier=${agent.tier} · codex sandbox=${agent.codexSandbox} · approval=${agent.codexApproval} -->\n\n`;
  return header + agent.body.trimStart();
}

function metaFile(agent) {
  return (
    `# Generated by the Newronaut installer from core/roster.json — do not hand-edit.\n` +
    `TIER=${agent.tier}\n` +
