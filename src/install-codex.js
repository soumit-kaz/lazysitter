...
const SKILL_DIR = '.codex/skills/aet';
...
function installCodex(ctx, data) {
  log.step('Codex adapter → .codex/skills/aet/');
  ...
  ctx.writePreserve(
    `${SKILL_DIR}/aet.config.json`,
    readFile(path.join(ctx.coreDir, 'codex', 'aet.config.json'))
  );

  // Discovery guarantee: point AGENTS.md at the skill.
  ctx.mergeMarkedBlock('AGENTS.md', readFile(path.join(ctx.coreDir, 'codex', 'AGENTS.aet.md')));
}

function roleFile(agent) {
  const header = `<!-- AET role: ${agent.name} · tier=${agent.tier} · codex sandbox=${agent.codexSandbox} · approval=${agent.codexApproval} -->\n\n`;
  return header + agent.body.trimStart();
}

function metaFile(agent) {
  return (
    `# Generated by the AET installer from core/roster.json — do not hand-edit.\n` +
    `TIER=${agent.tier}\n` +
