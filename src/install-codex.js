...
const SKILL_DIR = '.codex/skills/newton';
...
function installCodex(ctx, data) {
  log.step('Codex adapter → .codex/skills/newton/');
  ...
  ctx.writePreserve(
    `${SKILL_DIR}/newton.config.json`,
    readFile(path.join(ctx.coreDir, 'codex', 'newton.config.json'))
  );

  // Discovery guarantee: point AGENTS.md at the skill.
  ctx.mergeMarkedBlock('AGENTS.md', readFile(path.join(ctx.coreDir, 'codex', 'AGENTS.newton.md')));
}

function roleFile(agent) {
  const header = `<!-- Newton role: ${agent.name} · tier=${agent.tier} · codex sandbox=${agent.codexSandbox} · approval=${agent.codexApproval} -->\n\n`;
  return header + agent.body.trimStart();
}

function metaFile(agent) {
  return (
    `# Generated by the Newton installer from core/roster.json — do not hand-edit.\n` +
    `TIER=${agent.tier}\n` +
