...
const SKILL_DIR = '.codex/skills/provenforge';
...
function installCodex(ctx, data) {
  log.step('Codex adapter → .codex/skills/provenforge/');
  ...
  ctx.writePreserve(
    `${SKILL_DIR}/provenforge.config.json`,
    readFile(path.join(ctx.coreDir, 'codex', 'provenforge.config.json'))
  );

  // Discovery guarantee: point AGENTS.md at the skill.
  ctx.mergeMarkedBlock('AGENTS.md', readFile(path.join(ctx.coreDir, 'codex', 'AGENTS.provenforge.md')));
}

function roleFile(agent) {
  const header = `<!-- Provenforge role: ${agent.name} · tier=${agent.tier} · codex sandbox=${agent.codexSandbox} · approval=${agent.codexApproval} -->\n\n`;
  return header + agent.body.trimStart();
}

function metaFile(agent) {
  return (
    `# Generated by the Provenforge installer from core/roster.json — do not hand-edit.\n` +
    `TIER=${agent.tier}\n` +
