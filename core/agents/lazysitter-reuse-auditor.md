...
- The explorer's CONTEXT PACK (its ranked candidate sets and "Does this already exist?" section), for reference only — re-search yourself, don't just re-read its conclusion.
- The implementers' `## Precedent citations` rows, for reference only — you are the independent check on the claim, not a second transcription of it.
