...
- Artifacts: `REQUIREMENT.md`, `TRIAGE.md`, `CONTEXT-PACK.md`, `FACTS.tsv`, `ACCEPTANCE-CRITERIA.md`, `PLAN.md`, `DECISIONS.md`, and each verification report.
...
blocking_class: MINE|ENVIRONMENT|PRE-EXISTING   # C11 — whose fault; does NOT override the A1 degraded:true hard-BLOCK; only MINE blocks this diff
...
**Blocking class (C11).** `blocking_class:` records whose fault a finding is, not whether it blocks. Only a `MINE`-class finding blocks this diff's own merge; `ENVIRONMENT` and `PRE-EXISTING` route to standing disclosure in the final report instead. **This governs fault, not authority over A1: an unresolved `degraded:true` still hard-BLOCKs regardless of `blocking_class`**, and still needs the same recorded per-run human waiver — `blocking_class` never substitutes for it. Pipeline artifacts (`<run-dir>/`, `.lazysitter/`, `.claude/`, `.codex/`, `.cursor/`) are excluded from every product gate by construction — a finding inside them is never a reason to block a feature diff.
...
`lazysitter-recon` — not `explorer` — executes every `ASSERTION` and `PAIRED-POSITIVE` row (row shape 2) in `.lazysitter/knowledge/CONVENTIONS.md`, at Tier 0c, every run. A failing assertion or a zero-hit paired-positive flips that row to `STALE` before `explorer` builds the context pack; `STALE` rows are excluded from the pack, never silently trusted. `explorer`'s own Bash stays bound by the C5 probe allowlist (no exemption) and is never the executor of row-shape-2 assertions. **Because a tracked, committed file now supplies commands `recon` executes on every preflight, `CONVENTIONS.md` inherits `SECRETS-BASELINE.md`'s existing treatment: any diff touching it is flagged and NEVER auto-approved, even under `--auto`** (D-20) — say so explicitly whenever you report on a diff that touches it.
...
15. Re-read the frozen spec by path, then evaluate the gate mechanically from the structured data:
   ...
   - **Executable knowledge:** any diff in this run that touches `.lazysitter/knowledge/CONVENTIONS.md` itself is flagged and NEVER auto-approved, even under `--auto` — it is a committed file `recon` executes as code every run (C7/D-20), the same class of risk `SECRETS-BASELINE.md` carries.
