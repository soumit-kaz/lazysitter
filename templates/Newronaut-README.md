...
```
/newronaut Add CSV export to the analytics dashboard
```
...
**Kill switch:** create the file `.claude/newronaut/KILL` at any time. The orchestrator checks for it before each tier and halts immediately. Delete it to re-enable.
...
Every run writes to `.claude/newronaut/runs/<feature-slug>/`:
...
| Tier | Agents | Model |
|------|--------|-------|
| 0 Orchestration | orchestrator (the `/newronaut` command, main session) | Opus |
| 1 Intake | `newronaut-business-analyst`, `newronaut-triage` | Sonnet, Haiku |
| 2 Research | `newronaut-explorer` | Haiku |
| 3 Spec | `newronaut-spec-writer` | Sonnet |
| 4 Design | `newronaut-architect`, `newronaut-database-expert`, `newronaut-infra-expert`, `newronaut-frontend-expert`, `newronaut-security-expert`, `newronaut-ux-analyst`, `newronaut-devils-advocate` | Opus / Sonnet |
| 5 Build | `newronaut-backend-implementer`, `newronaut-frontend-implementer`, `newronaut-dependency-auditor` | Sonnet |
| 6 Verification | `newronaut-test-author`, `newronaut-test-runner`, `newronaut-code-reviewer`, `newronaut-red-team`, `newronaut-security-auditor`, `newronaut-secrets-scanner` | Sonnet / Opus / Haiku |
| 7 Integration & Intent | `newronaut-integration-checker`, `newronaut-closing-loop-auditor` | Sonnet, Opus |
| 8 Release & Recovery | `newronaut-release-agent`, `newronaut-monitor-agent`, `newronaut-rollback-agent`, `newronaut-docs-agent` | Sonnet / Haiku |
...
- Model tiers assume your plan can run Opus/Sonnet/Haiku subagents. If a tier isn't available, edit the `model:` field in the relevant `.claude/agents/newronaut-*.md` file.
