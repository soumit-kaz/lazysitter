...
`status` is exactly one of `live | STALE | unverifiable`.
...
preflight, every run — it already holds `Bash` and already re-executes every run (A1/C3). A
failing `ASSERTION`, or a zero-hit `PAIRED-POSITIVE`, flips the row's `status` before anything
downstream reads it. **`explorer` does NOT execute row-shape-2 assertions** — its Bash stays bound
by the C5 probe allowlist below, which excludes `dotnet build`, `npm test`, and any exit-code
test; no exemption is granted to it. Because assertion execution happens at Tier-0 from a
committed file, and — unlike the C5 allowlist below — those commands are deliberately
