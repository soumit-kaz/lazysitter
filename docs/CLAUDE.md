...
- **Orchestrator** = the `/aet` slash command (`.claude/commands/aet.md`), running in your
...
- **Agents** = `.claude/agents/aet-*.md`. Each has a `model:` (opus/sonnet/haiku) and a
...
| Tests blind to code | `aet-test-author` is spawned **in parallel** with the implementers, from the frozen spec — the implementation does not exist yet when tests are authored |

## Running

```
/aet <feature request> [--dry-run] [--budget <tokens>] [--auto]
```
...
Artifacts land in `.claude/aet/runs/<slug>/`. Kill switch: create `.claude/aet/KILL`.

## Changing models

Edit the `model:` field in the relevant `.claude/agents/aet-*.md`, or change `core/` and run
`aet update`. If your plan can't run a given tier, downgrade there — but keep the red-team on
