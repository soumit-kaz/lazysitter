...
- **Agents** = `.claude/agents/lazysitter-*.md`. Each has a `model:` (opus/sonnet/haiku) and a
...
| Tests blind to code | `lazysitter-test-author` is spawned **in parallel** with the implementers, from the frozen spec — the implementation does not exist yet when tests are authored |

## Running

```
/lsi <feature request> [--dry-run] [--budget <tokens>] [--auto]
```
...
`lazysitter update`. If your plan can't run a given tier, downgrade there — but keep the red-team on
