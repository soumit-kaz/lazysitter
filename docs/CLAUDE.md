...
- **Orchestrator** = the `/provenforge` slash command (`.claude/commands/provenforge.md`), running in your
...
- **Agents** = `.claude/agents/provenforge-*.md`. Each has a `model:` (opus/sonnet/haiku) and a
...
| Tests blind to code | `provenforge-test-author` is spawned **in parallel** with the implementers, from the frozen spec — the implementation does not exist yet when tests are authored |

## Running

```
/provenforge <feature request> [--dry-run] [--budget <tokens>] [--auto]
```
...
Artifacts land in `.claude/provenforge/runs/<slug>/`. Kill switch: create `.claude/provenforge/KILL`.

## Changing models

Edit the `model:` field in the relevant `.claude/agents/provenforge-*.md`, or change `core/` and run
`provenforge update`. If your plan can't run a given tier, downgrade there — but keep the red-team on
