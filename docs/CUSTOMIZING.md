# Customizing LazySitter

Everything derives from `core/`. Edit there and run `lazysitter update` in each installed project to
propagate — never hand-edit installed files (they're overwritten on update and listed in
`.lazysitter/manifest.json`). The two exceptions are the Codex `models.env` and `lazysitter.config.json`,
which are written once and preserved across updates.

## Change an agent's behavior

Edit the body of `core/agents/lazysitter-<name>.md`. The body is shared by both runtimes: Claude
...
```json
"lazysitter-red-team": { "tier": "high", "codexSandbox": "workspace-write", "codexApproval": "never", "distinctModel": true }
```
...
1. Add `core/agents/lazysitter-<name>.md` with `name`, `description`, `tools`, `model` frontmatter
