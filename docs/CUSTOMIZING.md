# Customizing Provenforge

Everything derives from `core/`. Edit there and run `provenforge update` in each installed project to
propagate — never hand-edit installed files (they're overwritten on update and listed in
`.provenforge/manifest.json`). The two exceptions are the Codex `models.env` and `provenforge.config.json`,
which are written once and preserved across updates.

## Change an agent's behavior

Edit the body of `core/agents/provenforge-<name>.md`. The body is shared by both runtimes: Claude
...
```json
"provenforge-red-team": { "tier": "high", "codexSandbox": "workspace-write", "codexApproval": "never", "distinctModel": true }
```
...
1. Add `core/agents/provenforge-<name>.md` with `name`, `description`, `tools`, `model` frontmatter
