#!/usr/bin/env bash
# Provenforge agent runner (Codex adapter).
# Launches ONE context-isolated `codex exec` for a single Provenforge agent, so that
...
#   <agent-name>   e.g. provenforge-architect  (matches agents/<name>.md + agents/<name>.meta)
...
[ -f "$ROLE" ] || { echo "provenforge: unknown agent '$AGENT' (no $ROLE)" >&2; exit 2; }
[ -f "$META" ] || { echo "provenforge: missing meta for '$AGENT' ($META)" >&2; exit 2; }
[ -f "$INPUTS" ] || { echo "provenforge: inputs file not found: $INPUTS" >&2; exit 2; }
...
if [ "$APPROVAL" = "on-request" ] && [ "${Provenforge_AUTO_GIT:-0}" = "1" ]; then
  APPROVAL="never"
fi
...
if [ "${Provenforge_DRY_PRINT:-0}" = "1" ]; then
