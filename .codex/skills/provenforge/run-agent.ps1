<#
Provenforge agent runner (Codex adapter) — PowerShell.
Launches ONE context-isolated `codex exec` for a single Provenforge agent.

Usage: pwsh run-agent.ps1 <agent-name> <inputs-file> <output-file>

Env:
  Provenforge_AUTO_GIT=1   downgrade on-request approval to `never` (headless auto-merge)
  Provenforge_DRY_PRINT=1  print the resolved codex command instead of running it
#>
param(
...
if (-not (Test-Path $role)) { Write-Error "provenforge: unknown agent '$Agent' (no $role)"; exit 2 }
if (-not (Test-Path $meta)) { Write-Error "provenforge: missing meta for '$Agent' ($meta)"; exit 2 }
if (-not (Test-Path $InputsFile)) { Write-Error "provenforge: inputs file not found: $InputsFile"; exit 2 }
...
if ($approval -eq 'on-request' -and $env:Provenforge_AUTO_GIT -eq '1') { $approval = 'never' }
...
if ($env:Provenforge_DRY_PRINT -eq '1') {
