#!/usr/bin/env bash
# Newronaut agent runner (Codex adapter).
# Launches ONE context-isolated `codex exec` for a single Newronaut agent, so that
...
#   <agent-name>   e.g. newronaut-architect  (matches agents/<name>.md + agents/<name>.meta)
...
[ -f "$ROLE" ] || { echo "newronaut: unknown agent '$AGENT' (no $ROLE)" >&2; exit 2; }
[ -f "$META" ] || { echo "newronaut: missing meta for '$AGENT' ($META)" >&2; exit 2; }
[ -f "$INPUTS" ] || { echo "newronaut: inputs file not found: $INPUTS" >&2; exit 2; }
...
if [ "$APPROVAL" = "on-request" ] && [ "${Newronaut_AUTO_GIT:-0}" = "1" ]; then
  APPROVAL="never"
fi
...
if [ "${Newronaut_DRY_PRINT:-0}" = "1" ]; then
