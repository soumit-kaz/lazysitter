<#
Newronaut agent runner (Codex adapter) — PowerShell.
Launches ONE context-isolated `codex exec` for a single Newronaut agent.

Usage: pwsh run-agent.ps1 <agent-name> <inputs-file> <output-file>

Env:
  Newronaut_AUTO_GIT=1   downgrade on-request approval to `never` (headless auto-merge)
  Newronaut_DRY_PRINT=1  print the resolved codex command instead of running it
#>
param(
...
if (-not (Test-Path $role)) { Write-Error "newronaut: unknown agent '$Agent' (no $role)"; exit 2 }
if (-not (Test-Path $meta)) { Write-Error "newronaut: missing meta for '$Agent' ($meta)"; exit 2 }
if (-not (Test-Path $InputsFile)) { Write-Error "newronaut: inputs file not found: $InputsFile"; exit 2 }
...
if ($approval -eq 'on-request' -and $env:Newronaut_AUTO_GIT -eq '1') { $approval = 'never' }
...
if ($env:Newronaut_DRY_PRINT -eq '1') {
