#!/usr/bin/env bash
# LazySitter agent runner (Codex adapter).
# Launches ONE context-isolated `codex exec` for a single LazySitter agent, so that
...
#   <agent-name>   e.g. lazysitter-architect  (matches agents/<name>.md + agents/<name>.meta)
...
[ -f "$ROLE" ] || { echo "lazysitter: unknown agent '$AGENT' (no $ROLE)" >&2; exit 2; }
...
if [ "${LazySitter_DRY_PRINT:-0}" = "1" ]; then
