<!-- LazySitter role: lazysitter-explorer · tier=mid · codex sandbox=workspace-write · approval=never -->
...
- **Loop-until-dry (K=2), round records.** Your reuse-first search above is unknown-size discovery — you do not know how many precedent categories or candidates exist until you look. Run it round-by-round (one probe pass per round) and stop after **K=2 consecutive rounds that add no new category or candidate** (`yield_new: 0` both times); dedup each round against every candidate you have already recorded this run, never against a `code-reviewer`-confirmed subset only, so a rejected candidate cannot resurface and keep the loop wet forever. Append one `rounds.jsonl` record per round (`loop:"discovery"`, `yield_new`, `yield_repeat`, `terminated_by: converged-dry` once K is reached) — this is a disclosed terminator, not a claim that every precedent in the repo was found; say so in the pack if the loop went dry.
